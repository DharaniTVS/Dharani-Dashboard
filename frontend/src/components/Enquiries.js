...
const Enquiries = ({ user, onLogout }) => {
  ...
  const applyFilters = () => {
    ...
    if (startDate && endDate) {
      filtered = filtered.filter(record => {
        const enquiryDate = record['Date'] || record['Enquiry Date'] || '';
        if (enquiryDate) {
          try {
        ...
        <div className="bg-white border-b border-gray-200 px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Enquiries - {selectedBranch}</h1>
                <p className="text-sm text-gray-600 mt-1">View customer enquiries from {selectedBranch} branch</p>
              </div>
            </div>
            <Button 
              variant="outline" 
